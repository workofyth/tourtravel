...
export async function FeaturedPackages() {
  ...
  const packages = await getFeaturedPackages(locale);
    ...
    <section className="py-20">
      <div className="container px-4 md:px-6 mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
          ...
          <ButtonLink href="/packages" variant="outline">{t("viewAll")}</ButtonLink>
        </div>
          ...
          {packages.map((pkg, idx) => (
            <Card key={pkg.id} className="overflow-hidden flex flex-col hover:shadow-lg transition-shadow">
              ...
              <CardFooter className="border-t border-muted pt-6 mt-auto">
                <ButtonLink href={`/packages/${pkg.slug}`} className="w-full">{t("details")}</ButtonLink>
              </CardFooter>
            </Card>
          ))}
        </div>
