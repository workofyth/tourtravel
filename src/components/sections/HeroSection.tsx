...
export function HeroSection({ packages = [] }: HeroSectionProps) {
  ...
  const staticSlides = [
    ...
    // },
    {
      image: "/petronas.jpg",
      title: "KUALA LUMPUR",
      subtitle: "THE HEART OF MALAYSIA",
      link: "/packages",
      hideContent: true
    }
  ];
  ...
  const activeSlides = packages.length > 0
    ? [
      ...packages.slice(0, 4).map(pkg => {
        ...
        const titleUpper = (pkg.title?.toUpperCase() || "UNTITLED PACKAGE").trim();
        const titleMatch = titleUpper.match(/^(.+?)\s*\((.+)\)\s*$/);
        const titleMain = titleMatch ? titleMatch[1].trim() : titleUpper;
        const titleDuration = titleMatch ? `(${titleMatch[2].trim()})` : "";

        return {
          image: imageUrl,
          title: titleMain,
          subtitle: titleDuration,
  ...
  const getTitleSizeClass = (title: string) => {
    ...
    if (len <= 20) return "text-2xl md:text-5xl lg:text-6xl";
    if (len <= 35) return "text-2xl md:text-4xl lg:text-5xl";
  ...
  return (
    <section className="relative w-full h-[90vh] overflow-hidden">
      ...
      {!activeSlides[currentSlide].hideContent && (
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-4">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="max-w-6xl"
            >
              <h1 className={`font-roboto text-white ${getTitleSizeClass(activeSlides[currentSlide].title)} font-black mb-3 leading-tight tracking-tighter drop-shadow-2xl px-0`}>
                {activeSlides[currentSlide].title}
              </h1>
              {activeSlides[currentSlide].subtitle && (
                <h2 className="font-roboto-slab text-white text-base md:text-xl font-bold mt-2 mb-6 tracking-wide uppercase drop-shadow-lg">
                  {activeSlides[currentSlide].subtitle}
                </h2>
              )}
