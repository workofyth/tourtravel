...
export default function FilterForm({
  ...
  categories: Category[], currentCategory?: string, currentSearch?: string
}) {
      ...
      <CardContent className="p-5 md:p-6">
        <form onSubmit={handleFilter} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 lg:gap-6 items-end">
          <div className="lg:col-span-5 space-y-1.5">
            <Label htmlFor="search" className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 ml-1">{t("keyword")}</Label>
            <div className="relative">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
              <Input id="search" type="text" placeholder={t("whereToGo")}
          ...
          <div className="lg:col-span-4 space-y-1.5">
            <Label htmlFor="category" className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 ml-1">{t("category")}</Label>
          ...
          <div className="lg:col-span-3 flex items-center gap-2 h-12">
            ...
            <Button type="button" variant="outline"
              ...
              onClick={() => { setCategory("all"); setSearch(""); router.push("/packages"); }}>
              {t("reset")}
            </Button>
          </div>
        </form>
